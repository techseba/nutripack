    ...
    <table class="w-full">
        <thead>
            <x-table.tr thead>
                ...
                <x-table.th class="w-10">photo</x-table.th>
        ...
        <tbody>
                    ...
                    <x-table.td>
                        @if ($row->image)
                            <img class="w-8 h-8 object-cover rounded-full" src="{{ asset('storage/' . $row->image) }}"
                                alt="{{ $row->name }}">
                        @else
                            <img class="w-8 h-8 object-cover rounded-full" src="{{ asset('assets/admin/user.png') }}"
                                alt="{{ $row->name }}">
                        @endif
                        <x-table.td>{!! highlight($row->name, $this->search) !!}</x-table.td>
                    </x-table.td>
                    <x-table.td>{!! highlight($row->phone, $this->search) !!}</x-table.td>
                    <x-table.td>{!! highlight($row->email, $this->search) !!}</x-table.td>
                    <x-table.td><x-widget.date-time :value="$row->updated_at" /></x-table.td>
                    <x-table.td>{!! highlight($row->getRoleNames()->first(), $this->search) !!}</x-table.td>
                    <x-table.td>
            ...
            @empty
                <x-table.tr>
                    <x-table.td colspan="9" class="text-center">{{ $subject ?? 'Item' }} was not
                        found.</x-table.td>
                </x-table.tr>
            @endforelse
