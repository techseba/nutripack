        ...
        <tbody>
            @forelse ($this->rows as $row)
                <x-table.tr>
                    ...
                    <x-table.td>
                        @if($row->image)
                            <img class="w-8 h-8 object-cover rounded-full" src="{{ asset('storage/' . $row->image) }}" alt="{{$row->name}}">
                        @else
            ...
            @empty
                <x-table.tr>
                    <x-table.td colspan="10" class="text-center">{{ ucfirst($subject) ?? 'Item' }} was not
                        found.</x-table.td>
                </x-table.tr>
            @endforelse
        </tbody>
