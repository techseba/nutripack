<div class="overflow-x-auto overflow-y-hidden scrollbar-custom max-w-full">
    <table class="w-full">
        <thead>
            <x-table.tr thead>
                <x-table.th class="w-10" scope="col">
                ...
                <x-table.th class="w-15" scope="col">ID</x-table.th>
                <x-table.th>date</x-table.th>
                <x-table.th>meal name</x-table.th>
                <x-table.th>meal type</x-table.th>
                <x-table.th>created at</x-table.th>
                <x-table.th class="w-20" scope="col">actions</x-table.th>
            </x-table.tr>
        </thead>
        <tbody>
            @forelse ($this->rows as $row)
                <x-table.tr wire:key="daywise-{{ $row->id }}">
                    <x-table.td>
                        <label class="relative inline-flex items-center cursor-pointer">
            ...
            @empty
                <x-table.tr>
                    <x-table.td colspan="7" class="text-center">{{ ucfirst($subject) ?? 'Item' }} was not
                        found.</x-table.td>
                </x-table.tr>
            @endforelse
        </tbody>
