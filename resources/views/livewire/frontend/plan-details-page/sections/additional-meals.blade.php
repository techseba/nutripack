@if ($selectedPlan)
    <fieldset class="flex flex-col gap-y-4 bg-white border border-slate-400 shadow-lg p-3 rounded-lg mb-8">
        ...
        @foreach ($additional_meals as $meal)
            <div
                ...
                <div class="col-span-1">
                    <input type="number" wire:model.live.debounce.150ms="qty.{{ $meal['name'] }}" min="0" max="{{ $meal->max_quantity }}" step="1"
                        class="w-full focus:outline-0 bg-white border border-slate-300 rounded-md py-1 px-2 font-bold" />
                </div>

                @php
                    $lineTotal = $meal->unit_price * $meal->max_quantity;
                @endphp

                <div class="col-span-4 text-right text-sm mt-1 pr-2">
                    <span class="text-slate-600">Line total: </span>
                    <span class="line-total font-bold">BHD {{ $lineTotal }} 0.00</span>
                </div>
            </div>
        @endforeach
        ...
        <pre>{{ json_encode($this->qty, JSON_PRETTY_PRINT) }}</pre>
