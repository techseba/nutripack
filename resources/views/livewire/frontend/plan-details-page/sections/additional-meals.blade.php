...
<fieldset class="flex flex-col gap-y-4 bg-white border border-slate-400 shadow-lg p-3 rounded-lg mb-8">
    ...
    @if ($this->hasLunch)
        <div
            ...
            <div class="col-span-2">
                ...
                <div class="text-xs text-slate-500">BHD {{ number_format($this->lunchUnitPrice, 2) }}</div>
            </div>

            <div class="col-span-1 text-sm text-right">
                <span class="text-slate-600">Max: {{ $this->lunchMaxQuantity }}</span>
            </div>

            <div class="col-span-1">
                <input type="number" wire:model.live.debounce.150ms="lunchQuantity" min="0" max="{{ $this->lunchMaxQuantity }}"
                    step="1"
                    class="w-full focus:outline-0 bg-white border border-slate-300 rounded-md py-1 px-2 font-bold" />
            </div>
            ...
            <div class="col-span-4 text-right text-sm mt-1 pr-2">
                <span class="text-slate-600">Line total: </span>
                <span class="line-total font-bold">BHD {{ number_format($this->lunchTotalPrice, 2) }}</span>
            </div>
    ...
    <div
        ...
        <label class="col-span-2 font-medium">Per day additional price</label>
        <span id="additional-total" class="col-span-1 font-bold text-right">BHD {{ number_format($this->totalAdditionalPrice, 2) }}</span>
    </div>
