    ...
    <div x-show="modalOpen" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="">
            ...
            <div class="p-6 space-y-2">
                ...
                <h2 class="font-medium text-lg">Contains</h2>
                <div class="grid grid-cols-4 gap-2 h-25 overflow-y-auto">

                    @forelse ($this->ingredients as $ingredient)
                        <div class="p-4 text-center bg-slate-50 rounded-2xl">
                            <p class="text-[10px] font-black text-slate-400 uppercase mb-1">{{ $ingredient->name }}</p>
                            @if ($ingredient->image)
                                <img class="w-10 h-10 mx-auto object-cover" src="{{ asset('storage/'. $ingredient->image) }}" alt="{{ $ingredient->name }}">
                            @else
